...
const Adminordercard = ({ order }: { order: IOrder }) => {
    ...
    const updateStatus=async(orderId:string,status:string)=>{
        try {
            const reult=await axios.post(`/api/admin/update_order_status/${orderId}`,{status})
    ...
    return (
        <motion.div
            ...
            className='bg-white shadow-md hover:shadow-lg  border border-gray-100 rounded-2xl p-4 transition-all '
        >
            <div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4'>
                <div className='space-y-1'>
                    ...
                    {status !== "delivered" && <span className={`inline-block text-xs font-semibold px-3 py-1 rounded-full border ${order.isPaid ? "bg-green-100 text-gray-700 border-green-300" : "bg-red-100 text-red-700 border-red-300"}`}>{order.isPaid ? "Paid" : "Unpaid"}</span>}
                    ...
                    {order.assignedDeliveryBoy && <div className='mt-4 bg-blue-50 border border-blue-200 p-4 rounded-xl flex items-center justify-between'>
                ...
                <span className={`text-xs font-semibold px-3 py-1 rounded-full capitalize ${status === "delivered" ? "bg-green-100 text-green-700": status === "pending" ? "bg-yellow-100 text-yellow-700" :"bg-blue-100 text-blue-700"}`}>
                {status}
                </span>
                {status != "delivered" && <select className='border border-gray-300 rounded-lg px-3 py-1 text-sm shadow-sm hover:border-green-400 transition focus:ring-2 focus:ring-green-500 outline-none' value={order.status}  onChange={(e)=>updateStatus(order._id?.toString()!, e.target.value)}>
                {statusOptions.map(st=>(
                    <option className="text-sm" key={st} value={st}>{st.toUpperCase()}</option>
                ))}
                </select>
}
            ...
            <div className='border-t border-gray-200 mt-3 pt-3'>
                ...
                <div className='border-t pt-3 mt-3 flex justify-between items-center text-sm font-semibold text-gray-800'>
                    <div className='flex items-center gap-2 text-gray-700 text-sm'>
                        ...
                        <span className='text-green-700 font-semibold'>Delivery:<span>{status}</span> </span>

                    </div>
                    <div>
                        Total: <span className='text-green-700 font-bold'>₹{Number(order.totalAmount)}</span>
                    </div>
