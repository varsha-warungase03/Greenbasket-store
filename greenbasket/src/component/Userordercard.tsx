...
const Userordercard = ({ order }: { order: IOrder }) => {
    ...
    const [status, setStatus] = useState(order.status.trim().toLowerCase())
    ...
    return (
        <motion.div className='bg-white rounded-2xl border border-gray-100 shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden'>
            <div className='flex flex-col md:flex-row justify-baseline items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-linear-to-r from-green-100 to-white'>
                <div>
                ...
                {order.assignedDeliveryBoy && <> <div className='mt-4 bg-blue-50 border border-blue-200 p-4 rounded-xl flex items-center justify-between'>
                    ...
                    <button className='w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2 rounded-xl shadow hover:bg-green-700 transition ' onClick={() => {
                        if (!order?._id) return;
                        router.push(`/user/track_order/${order._id}`);
