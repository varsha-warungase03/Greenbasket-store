...
const RegisterForm = ({previousStep}:propType) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-linear-to-b from-green-200 to-white'>
        ...
        <p className='text-gray-600 flex items-center'>Join GrenBasket today <Leaf className='w-6 h-6'/></p>
        ...
        <input type='text' placeholder=' Your Name' className='w-full border-gray-300 rounded-xl py-3 pl-10 pr-4 text-gray-800 focus:ring-2 focus:ring-green-500 focus:outline-none bg-gray-100' onChange={(e)=>setName(e.target.value)} value={name}/>
        </div>
        <div className='relative'>
        ...
        <p className='text-gray-600 mt-6 text-sm flex items-center gap-1' onClick={()=>router.push("/login")}>Already have an Account ? <LogIn className='h-4 w-4' /><span className='text-green-700 cursor-pointer'>Sign in</span></p>
