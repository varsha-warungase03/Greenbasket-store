...
const Checkout = () => {
    ...
    const DraggableMarker:React.FC=()=>{
        ...
        return <Marker position={position as LatLngExpression} icon={markerIcon}  draggable={true}
        eventHandlers={{dragend:(e:L.LeafletEvent)=>{
    ...
    const handleSearchquery = async () => {
        ...
        try {
            const response = await axios.get(`/api/geocode?q=${encodeURIComponent(searchQuery)}`);
    ...
    return (
        <div className='w-[92%] md:w-[80%] mx-auto py-10 relative'>
            <motion.button
                whileTap={{ scale: 0.97 }}
                className='absolute left-0 top-4 flex items-center gap-2 text-green-700 hover:text-green-800 font-semibold' onClick={() => router.push("/user/cart")}
            >
                <ArrowLeft size={16} />
            ...
            <div className='grid md:grid-cols-2 gap-8'>
                ...
                >
                    ...
                    <div className='space-y-4'>
                        <div className='relative'>
                            ...
                            <input type='text' placeholder='Full Name' value={address.fullName || ""} onChange={(e) => setAddress((prev) => ({ ...prev, fullName:e.target.value}))}
                                className='pl-10 w-full border rounded-lg p-3 text-sm bg-gray-50' />
                        </div>
                        <div className='relative'>
                            <Phone className='absolute left-3 top-3 text-green-600' size={18} />
                            <input type='text' placeholder='09876XXXXX' value={address.mobile || ""} onChange={(e) => setAddress((prev) => ({ ...prev, mobile: e.target.value}))}
                                className='pl-10 w-full border rounded-lg p-3 text-sm bg-gray-50'/>
                        </div>
                        <div className='relative'>
                            <Home className='absolute left-3 top-3 text-green-600' size={18} />
                            <input type='text' placeholder='Full address..' value={address.fullAddress || ""} onChange={(e) => setAddress((prev) => ({ ...prev, fullAddress: e.target.value }))}
                                className='pl-10 w-full border rounded-lg p-3 text-sm bg-gray-50' />
                        </div>
                        <div className='grid grid-cols-3 gap-3'>
                            ...
                            <div className='relative'>
                                <Navigation className='absolute left-3 text-green-600 top-3' size={18} />
                        ...
                        <div className='flex gap-2 mt-3'>
                            <input type='text' placeholder='serach city or area..' className='flex-1 border rounded-lg p-3 text-sm focus:ring-2 focus:ring-green-500 outline-none' value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)}/>
                            <button className='bg-green-600 text-white px-5 rounded-lg hover:bg-green-700 transition-all font-medium' onClick={handleSearchquery}>{searchLoading ? <Loader2 className='animate-spin' size={16}/> : "Search"}</button>
                        </div>

                        <div className='relative mt-6 h-82.5 rounded-xl overflow-hidden border border-gray-200 shadow-inner'>
                            {position && 
                            <MapContainer center={position as LatLngExpression} zoom={17} scrollWheelZoom={true} className='w-full h-full'>
                                <TileLayer
                ...
                >
                    ...
                    className={`flex items-center gap-3 w-full border rounded-lg p-3 transition-all ${
                        ...
                        ? "border-green-600 bg-green-50 shadow-sm" : "hover:bg-gray-50"
                    }`}>
                    ...
                    <motion.button
                    whileTap={{scale:0.9}}
                    className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all font-semibold' onClick={()=>{
